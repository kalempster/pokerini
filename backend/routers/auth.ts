import { env, publicProcedure, prisma, t } from "..";
...
export const authRouter = t.router({
  ...
  login: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const REFRESH_TOKEN = jwt.sign({ id: user.id }, env.JWT_REFRESH_SECRET);
  ...
  logout: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      try {
        await prisma.token.delete({ where: { token: input.REFRESH_TOKEN } });
      } catch (error) {
        throw new TRPCError({
