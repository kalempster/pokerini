...
import { CloseLobby, LeaveLobby, StartGame, SyncLobby } from "@gameserver/shared/messages";
...
const Lobby = () => {
    ...
    const closeLobby = () => {
        client.send(CloseLobby, { lobbyId: gameStore.id }, undefined);
    };
    ...
    return (
        <div className="h-[100lvh]">
            ...
            <Section className="flex flex-col items-center justify-center gap-5 pt-[calc(var(--header-height)*1.5)] tall:pt-0">
                ...
                {gameStore.spectators.length > 0 ? (
                    <div className="flex w-1/3 flex-col items-center justify-center gap-4">
                        <div className="text-primary">Spectators:</div>
                        {gameStore.spectators.map((p, index) => (
                            <LobbyPlayer
                                id={p.id}
                                username={p.username}
                                chips={0}
